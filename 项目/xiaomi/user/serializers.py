...
from user.models import XiaomiUser, Address
...
class UserRegisterSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if XiaomiUser.objects.filter(u_phone=phone).exists():
            raise XiaomiException({"code":1000,"msg":"该用户名已存在!"})
        if not password == password2:
            raise XiaomiException({"code":1001,"msg":"两次密码不一致"})
        if not u_captcha == captcha:
            raise XiaomiException({"code": 1004, "msg": "验证码错误！"})
        return attrs
...
class UserLoginSerializer(serializers.Serializer):
    ...
    def validate(self,attrs):
        if not XiaomiUser.objects.filter(u_phone=attrs.get("uphone")).exists():
            raise XiaomiException({"code":2000,"msg":"手机号不正确"})

        user = XiaomiUser.objects.filter(u_phone=attrs.get("uphone")).first()
        if not check_password(attrs.get("upassword"),user.u_password):
            raise XiaomiException({"code":2001,"msg":"登陆密码不正确"})
        return attrs
...
class UserCaptchaSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not re.match(r"1[3-9]\d{9}$", phone):
            raise XiaomiException({"code": 3001, "msg": "手机号有误！"})
        return attrs
