...
class UserViewSet(viewsets.GenericViewSet,mixins.ListModelMixin):
    queryset = XiaomiUser.objects.all()
    ...
    def list(self, request, *args, **kwargs):
        ...
        if user_id:
            user = XiaomiUser.objects.filter(id=user_id).first()
            ...
            raise XiaomiException({"code":3001,"msg":"用户信息已过期，请重新登陆"})
    ...
    def register(self,request):
        ...
        if not is_valid:
            raise XiaomiException({"code":1003,"msg":"注册数据个数不正确"})
        u_phone = serializer.data["uphone"]
        u_password = serializer.data["upassword"]
        userpassword = make_password(u_password)
        new_user = XiaomiUser.objects.create(u_phone=u_phone,u_name=u_phone,u_password=userpassword)
        result = {
            "user_id":new_user.id
        }
        return Response(result)
    ...
    def login(self,request):
        ...
        is_valid = serializer.is_valid(raise_exception=False)
        if not is_valid:
            raise XiaomiException({"code":2002,"msg":"登录数据有误"})
        user = XiaomiUser.objects.filter(u_phone=serializer.data["uphone"]).first()
    ...
    def captcha(self,request):
        ...
        if not is_valid:
            raise XiaomiException({"code": 1, "msg": "手机号验证有误"})
        ...
        print(captcha)
        if not captcha:
            raise XiaomiException({"code": 2, "msg": "验证码获取失败！"})
    ...
    def findpassword(self,request):
        ...
        is_valid = ser.is_valid(raise_exception=False)
        if not is_valid:
            raise XiaomiException({"code": 1, "msg": "邮箱验证有误"})
        u_phone = ser.data["uphone"]
        u_email = ser.data["uemail"]
        user = XiaomiUser.objects.filter(u_phone=u_phone,u_email=u_email).first()
        token = send_captcha_email(u_email)  # 发送邮件，获取验证码
        if not token:
            raise XiaomiException({"code": 2, "msg": "邮件发送失败！"})
    ...
    def resetpassword(self,request):
        ...
        if request.method == "POST":
            ...
            is_valid = ser.is_valid(raise_exception=False)
            if not is_valid:
                raise XiaomiException({"code": 102 ,"msg": "密码重置失败"})
...
class AddressViewSet(viewsets.GenericViewSet,mixins.ListModelMixin,mixins.UpdateModelMixin):
    ...
    def addaddress(self,request):
        ...
        if user:
            ...
            if not is_valid:   #格式验证不通过
                raise XiaomiException({"code": 4000, "msg": "数据格式有误"})
            ...
            raise XiaomiException({"code": 5000, "msg": "请先登录"})
    ...
    def deladdress(self,request):
        ...
        if address:
            ...
            raise XiaomiException({"code": 4001, "msg": "没有该地址，无法删除"})
