...
from xiaomi.settings import app_private_key_string, alipay_public_key_string
from carts.models import Cart
from orders.filter import OrderFilter
from orders.models import Order, OrderDetail
from orders.serializers import OrderSerializer
from user.models import Address, XiaomiUser
...
class OrderViewSet(viewsets.GenericViewSet,mixins.ListModelMixin,mixins.CreateModelMixin):
    ...
    def addorder(self, request):
        ...
        if carts:  #购物车中有选定的商品
            ...
            raise XiaomiException({"code": "7000", "msg": "没有选中商品，无法下单"})
    ...
    def delorder(self,request):
        ...
        if order:   #存在该订单
            ...
            raise XiaomiException({"code": "7001", "msg": "没有该订单，无法取消"})
...
class OrderPayView(View):
    ...
    def post(self, request):
        try:
            ...
            raise XiaomiException({"code": 5001, "msg": "未登录，请先登录！！！"})

        o_no = request.POST.get("ono")
        if not o_no:
            raise XiaomiException({"code": 8002, "msg": "无效的订单号！"})
        try:
            order = Order.objects.filter(o_no=o_no, o_user=user, o_state=1).first()
        except Order.DoesNotExist:
            raise XiaomiException({"code": 8003, "msg": "订单错误！"})
